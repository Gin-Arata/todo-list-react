const NavbarLayouts = (props) => {
  ...
  return (
    <div className="max-w-full min-h-10 bg-bgcolorcardnavbar flex items-end gap-x-1">
      {/* when selected */}
      <div className={`min-h-8 w-48 rounded-t-xl flex justify-between items-center ${titleNavbar === "home" ? "bg-bgcolorcard font-semibold" : "bg-bgcolorcardnavbar hover:bg-bgcolorcard"}`}>
        <p className="px-2 text-slate-800 text-sm">Home</p>
        <i className="fa fa-times-circle text-slate-300 px-2"></i>
      </div>
      {/* when not selected */}
      <div className={`min-h-8 w-48 rounded-t-xl flex justify-between items-center ${titleNavbar === "???" ? "bg-bgcolorcard font-semibold" : "bg-bgcolorcardnavbar hover:bg-bgcolorcard"}`}>
        <p className="px-2 text-slate-800 text-sm">Home</p>
        <i className="fa fa-times-circle text-slate-300 px-2"></i>
      </div>
